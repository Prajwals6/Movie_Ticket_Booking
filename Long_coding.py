...
class UserFunctionality:
    def validate_user(self, mail_id, password=None):
        ...
        if not registered_mail_id and password == None:
            ...
            varified_password  = [user.password for user in UserData.users if user.password == password]
            if registered_mail_id and varified_password:
                return True
            
    def signup(self):
        user_id = (UserData.users[-1].user_id)+1
        user_name = input("Enter user name:\n").lower()
        mail_id = input("Enter the Mial Id:\n").lower
        phone_no = input("Enter a Mobile Number:\n")
        password = input("Enter a strong password:\n")

        if self.validate_user(mail_id):
            new_user = UserData(user_id, user_name, mail_id, phone_no, password)
...
if __name__ == '__main__':
    user1 = UserData(1, 'Prajwals6', 'prajwalyash6@gmail.com', '7349120536', 'Prajwals6')
    user2 = UserData(2, 'Venkatesh7', 'venky7@gmail.com', '9916108356', 'Venkatesh7')
    user3 = UserData(3, 'Wakanda1', 'wakanda1@gmail.com', '6366107678', 'Wakanda1')
